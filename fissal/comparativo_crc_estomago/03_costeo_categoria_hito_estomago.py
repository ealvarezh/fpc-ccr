...
ARCHIVO_HITOS = SILVER / "FISSAL_ESTOMAGO_HITOS_PACIENTE.parquet"
...
print("=" * 70)
print("COSTEO POR HITO Y SUBCATEGORIA — CANCER DE ESTOMAGO (C16)")
print("=" * 70)
print("""
Mismas ventanas de costo que CRC:
  1. DESPISTAJE   : toda actividad ANTES del diagnostico.
  2. DIAGNOSTICO  : mismo dia del diagnostico.
  3. TRATAMIENTO  : (diagnostico, fin de tratamiento] — solo con tratamiento
                     detectado.
  4. DESENLACE    : (fin de tratamiento, desenlace] si tuvo tratamiento, o
                     (diagnostico, desenlace] si no. FECHA_DESENLACE aqui es
                     la version simplificada (fallecimiento o ultima
                     atencion, sin recaida/paliativo).
""")
...
pac_est = set(hp["NUM_DOC_CRYPTO"])
fechas = hp.set_index("NUM_DOC_CRYPTO")
print(f"Pacientes estomago: {len(hp):,}")
...
COLS_SALIDA = ["NUM_DOC_CRYPTO", "HITO", "categoria_recurso_502", "subcategoria_recurso_502", "ATE_MONTONETO"]
registros_hito = []
...
print("\nCargando data de estomago (ya filtrada) para Hitos 2, 3 y 4...")
est = pd.read_parquet(ARCHIVO_EST, columns=["NUM_DOC_CRYPTO", "ATE_FECATENCION", "ATE_DESCCONSUMO", "ATE_MONTONETO"])
est = est.merge(
...
print(f"  Registros estomago: {len(est):,}")
est = clasificar(est)

tuvo_trat = est["SECUENCIA_TRATAMIENTO"] != "SIN_TRATAMIENTO_DETECTADO"

print("  Hito 2 (dia del diagnostico)...")
m2 = est["ATE_FECATENCION"] == est["FECHA_DIAGNOSTICO"]
h2 = est.loc[m2].copy()
...
m3 = tuvo_trat & (est["ATE_FECATENCION"] > est["FECHA_DIAGNOSTICO"]) & \
    (est["ATE_FECATENCION"] <= est["FECHA_FIN_TRATAMIENTO"])
h3 = est.loc[m3].copy()
...
inicio_4 = pd.to_datetime(np.where(tuvo_trat, est["FECHA_FIN_TRATAMIENTO"], est["FECHA_DIAGNOSTICO"]))
est["_INICIO_HITO4"] = inicio_4
m4 = (est["ATE_FECATENCION"] > est["_INICIO_HITO4"]) & (est["ATE_FECATENCION"] <= est["FECHA_DESENLACE"])
h4 = est.loc[m4].copy()
h4["HITO"] = "4_DESENLACE"
...
# --- Ventanas de fecha/duracion por hito ---
ventanas = []
...
out_hito = SILVER / "FISSAL_ESTOMAGO_COSTEO_HITO.parquet"
out_subcat = SILVER / "FISSAL_ESTOMAGO_COSTEO_HITO_SUBCATEGORIA.parquet"
